...
lazy_static! {
    static ref IDT: InterruptDescriptorTable = {
        ...
        idt.simd_floating_point.set_handler_fn(simd_floating_handler);
...
extern "x86-interrupt" fn divide_errror_handler(stack_frame: InterruptStackFrame) {
...
extern "x86-interrupt" fn general_protection_handler(stack_frame: InterruptStackFrame, err_code: u64) {
    panic!("EXCEPTION: GENERAL PROTECTION FAULT\n{:#?}\nERR_CODE: {}", stack_frame, err_code); 
}

extern "x86-interrupt" fn x87_floating_handler(stack_frame: InterruptStackFrame) {
    println!("EXCEPTION: x87 FLOATING POINT EXCEPTION\n{:#?}", stack_frame); 
}
...
extern "x86-interrupt" fn simd_floating_handler(stack_frame: InterruptStackFrame) {
...
extern "x86-interrupt" fn cp_protection_handler(stack_frame: InterruptStackFrame, err_code: u64) {
...
extern "x86-interrupt" fn vmm_communication_handler(stack_frame: InterruptStackFrame, err_code: u64) {
    println!("EXCEPTION: VMM COMMUNICATION EXCEPTION\n{:#?}\nERR_CODE: {}", stack_frame, err_code); 
}

extern "x86-interrupt" fn security_handler(stack_frame: InterruptStackFrame, err_code: u64) {
...
extern "x86-interrupt" fn double_fault_handler(
    ...
    panic!("EXCEPTION: DOUBLE FAULT\n{:#?}\nERR_CODE: {}", stack_frame, _err_code); 
}
    ...
    _stack_frame: InterruptStackFrame)
{
    ...
    lazy_static!{
        static ref KEYBOARD: Mutex<Keyboard<layouts::No105Key, ScancodeSet1>> = {
            Mutex::new(
                Keyboard::new(ScancodeSet1::new(), layouts::No105Key, HandleControl::Ignore)
            )
        };
    }
